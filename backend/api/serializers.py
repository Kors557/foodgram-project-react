...
class RecipeListSerializer(serializers.ModelSerializer):
    ...
    ingredients = IngredientRecipeListSerializer(many=True, source="ingredient_recipe")
...
class RecipeCreateSerializer(serializers.ModelSerializer):
        ...
        for ingredient in ingredients:
            ...
            if int(amount) <= 0:
                raise serializers.ValidationError(
                    "Убедитесь, что значение количества " f'ингредиента "{ingredient_obj.name}" больше 0'
                )
            if ingredient_obj.id in ingredients_list:
                raise serializers.ValidationError(
                    f'Ингредиент "{ingredient_obj.name}" ' "в рецепте не должен повторяться."
                )
    ...
    def create(self, validated_data):
        ...
        self.ingredients_creation(recipe, ingredients_data)
        return recipe

    def update(self, recipe, validated_data):
        if 'ingredients' in validated_data:
            ingrediens = validated_data.pop('ingredients')
            recipe.ingrediens.clear()
            self.ingredients_creation(ingrediens, recipe)
